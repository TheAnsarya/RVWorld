...
namespace DATReader.Utils {
	public static class DatFindParentSets {
		public static void FindParentSet(DatDir searchGame, DatDir parentDir, bool includeBios, ref List<DatDir> lstParentGames) {
			...
			var parentName = searchGame.DGame.RomOf;
			...
			if (parentDir.ChildNameSearch(new DatDir(searchGame.DatFileType) { Name = parentName }, out var intIndex) != 0) {
				return;
			}

			var parentGame = (DatDir)parentDir.Child(intIndex);
