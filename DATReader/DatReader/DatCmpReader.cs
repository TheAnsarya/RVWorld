...
namespace DATReader.DatReader {
	public static class DatCmpReader {
		public static bool ReadDat(string strFilename, ReportError errorReport, out DatHeader datHeader) {
			using (var dfl = new DatFileLoader()) {
				datHeader = new DatHeader { BaseDir = new DatDir(DatFileType.UnSet) };
				var errorCode = dfl.LoadDat(strFilename, DatRead.Enc);
				if (errorCode != 0) {
				...
				while (!dfl.EndOfStream()) {
					var res = ReadNextBlock(dfl, datHeader.BaseDir, errorReport);
					if (!res) {
						return false;
					}
				}
		...
		private static bool LoadDirFromDat(DatFileLoader dfl, DatDir parentDir, ReportError errorReport) {
			...
			while (dfl.Next != ")") {
				var res = ReadNextBlock(dfl, dir, errorReport);
				if (!res) {
					return false;
				}
			}
		...
		private static bool LoadGameFromDat(DatFileLoader dfl, DatDir parentDir, ReportError errorReport) {
			...
			dfl.Gn();
			var dDir = new DatDir(DatFileType.UnSet) { Name = name, DGame = new DatGame() };
			...
			while (dfl.Next != ")" && !dfl.EndOfStream()) {
				switch (dfl.Next.ToLower()) {
					case "romof":
						...
						break;
					case "description":
						var description = dfl.GnRestQ();
						var idx1 = description.IndexOf("\"");
						if (idx1 != -1) {
							var idx2 = description.IndexOf("\"", idx1 + 1);
							if (idx2 != -1) {
			...
			while (dfl.Next != ")") {
				switch (dfl.Next.ToLower()) {
					...
					case "merge":
						...
						break;
					case "flags":
						var flags = VarFix.ToLower(dfl.Gn());
