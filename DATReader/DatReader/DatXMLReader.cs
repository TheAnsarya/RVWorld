...
namespace DATReader.DatReader {
	public static class DatXmlReader {
		...
		public static bool ReadDat(XmlDocument doc, string strFilename, out DatHeader datHeader) {
			...
			var dirNodeList = doc.DocumentElement.SelectNodes("dir");
			if (dirNodeList != null) {
				for (var i = 0; i < dirNodeList.Count; i++) {
					LoadDirFromDat(datHeader.BaseDir, dirNodeList[i]);
				}
			}

			var gameNodeList = doc.DocumentElement.SelectNodes("game");
		...
		private static bool LoadHeaderFromDat(XmlDocument doc, string filename, DatHeader datHeader) {
			...
			var packingNode = head.SelectSingleNode("romvault") ?? head.SelectSingleNode("clrmamepro");
			...
			var notzipped = VarFix.String(head.SelectSingleNode("notzipped"));
		...
		private static void LoadDirFromDat(DatDir parentDir, XmlNode dirNode) {
			...
			var name = VarFix.String(dirNode.Attributes.GetNamedItem("name"));
			...
			var dirNodeList = dirNode.SelectNodes("dir");
			if (dirNodeList != null) {
				for (var i = 0; i < dirNodeList.Count; i++) {
			...
			var gameNodeList = dirNode.SelectNodes("game");
			if (gameNodeList != null) {
				for (var i = 0; i < gameNodeList.Count; i++) {
		...
		private static void LoadGameFromDat(DatDir parentDir, XmlNode gameNode) {
			...
			var emuArc = gameNode.SelectSingleNode("tea") ?? gameNode.SelectSingleNode("trurip") ?? gameNode.SelectSingleNode("EmuArc");
			if (emuArc != null) {
				dGame.IsEmuArc = true;
			...
			var romNodeList = gameNode.SelectNodes("rom");
			if (romNodeList != null) {
				for (var i = 0; i < romNodeList.Count; i++) {
					LoadRomFromDat(dDir, romNodeList[i]);
				}
			}

			var diskNodeList = gameNode.SelectNodes("disk");
			if (diskNodeList != null) {
				for (var i = 0; i < diskNodeList.Count; i++) {
					LoadDiskFromDat(dDir, diskNodeList[i]);
				}
			}

			var deviceRef = gameNode.SelectNodes("device_ref");
			if (deviceRef != null) {
				for (var i = 0; i < deviceRef.Count; i++) {
		...
		private static void LoadRomFromDat(DatDir parentDir, XmlNode romNode) {
			...
			var rvRom = new DatFile(DatFileType.UnSet) {
		...
		private static void LoadDeviceRef(DatGame dGame, XmlNode deviceNode) {
			...
			var name = VarFix.String(deviceNode.Attributes.GetNamedItem("name"));
