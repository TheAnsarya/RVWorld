...
namespace DATReader.DatClean {
	public static class DatSetCompressionType {
		public static void SetZip(DatBase inDat, bool is7Zip = false) => SetZip(inDat, is7Zip, new List<DatDir>());
		...
		private static void SetZip(DatBase inDat, bool is7Zip, List<DatDir> parents) {
			...
			if (inDat is DatFile dFile) {
				if (dFile.isDisk) {
					...
					var dir = parents[parentCount - 2];
					var zipDir = parents[parentCount - 1];

					var tmpFile = new DatDir(DatFileType.Dir) { Name = zipDir.Name, DGame = zipDir.DGame };

					if (dir.ChildNameSearch(tmpFile, out var index) != 0) {
