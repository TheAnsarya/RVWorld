...
namespace DATReader.DatWriter {
	public static class DatXMLWriter {
		public static void WriteDat(string strFilename, DatHeader datHeader, bool newStyle = false) {
			...
			using (var sw = new DatStreamWriter(strFilename)) {
		...
		public static void WriteDat(System.IO.Stream strOut, DatHeader datHeader, bool newStyle = false) {
			using (var sw = new DatStreamWriter(strOut)) {
				WriteToStream(sw, datHeader, newStyle);
			}
		}
		...
		private static void writeBase(DatStreamWriter sw, DatDir baseDirIn, bool newStyle) {
			var dirChildren = baseDirIn.ToArray();

			if (dirChildren == null) {
				return;
			}

			foreach (var baseObj in dirChildren) {
		...
		private static string ByteToStr(byte[] b) => b == null ? "" : BitConverter.ToString(b).ToLower().Replace("-", "");

		private class DatStreamWriter : IDisposable {
			private int _tabDepth;
			private string _tabString = "";
			private readonly System.IO.StreamWriter _sw;
			public DatStreamWriter(string path) => _sw = File.CreateText(path);
			public DatStreamWriter(System.IO.Stream stOut) => _sw = new System.IO.StreamWriter(stOut);
			...
			public void WriteLine(string value) => _sw.WriteLine(_tabString + value);
			public void Write(string value) => _sw.Write(_tabString + value);
			...
			public void WriteEnd(string value) => _sw.WriteLine(value);
