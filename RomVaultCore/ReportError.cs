...
namespace RomVaultCore {
	public static class ReportError {
		...
		private static void SendErrorMessage(string message) {
			...
			var e = new EndpointAddress(@"http://services.romvault.com/RVService.svc");
			var s = new RVServiceClient(b, e);
		...
		private static void OpenLog() {
			...
			var logFilename = GetLogFilname();
			_logStreamWriter = new StreamWriter(logFilename, true);
		}

		private static void ReportFile(TextWriter sw, RvFile f) => sw.WriteLine($"{f?.Size?.ToString().PadRight(12)} {f?.CRC?.ToHexString()} {f?.AltSize?.ToString().PadRight(12)} {f?.AltCRC?.ToHexString()} {f?.HeaderFileType} {f?.GotStatus.ToString().PadRight(10)} {f?.RepStatus.ToString().PadRight(15)} {f?.TreeFullName}");
