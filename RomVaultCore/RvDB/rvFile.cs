	...
	public class RvFile {
		...
		public RepStatus RepStatus {
			...
			set {
				...
				var rs = RepairStatus.StatusCheck[(int)FileType, (int)_datStatus, (int)_gotStatus];
		...
		public void Write(BinaryWriter bw) {
			...
			var countChild = _children?.Count ?? 0;

			FileFlags fFlags = 0;
			if (Tree != null) {
				fFlags |= FileFlags.HasTree;
			}

			if (Game != null) {
				fFlags |= FileFlags.HasGame;
			}

			if (Dat != null) {
				fFlags |= FileFlags.HasDat;
			}

			if (countDirDats > 0) {
				fFlags |= FileFlags.HasDirDat;
			}

			if (countChild > 0) {
				fFlags |= FileFlags.HasChildren;
			}
			...
			if (HeaderFileType != HeaderFileType.Nothing) {
				fFlags |= FileFlags.HeaderFileType;
			}

			if (AltSize != null) {
				fFlags |= FileFlags.AltSize;
			}

			if (AltCRC != null) {
				fFlags |= FileFlags.AltCRC;
			}

			if (AltSHA1 != null) {
				fFlags |= FileFlags.AltSHA1;
			}

			if (AltMD5 != null) {
				fFlags |= FileFlags.AltMD5;
			}
			...
			if (DatModTimeStamp != null) {
				fFlags |= FileFlags.DatModTimeStamp;
			}
#endif
...
#if dt
			if (DatModTimeStamp != null) {
				bw.Write((ulong)DatModTimeStamp);
			}
#endif
			...
			if (CRC != null) {
				bw.WriteByteArray(CRC);
			}

			if (SHA1 != null) {
				bw.WriteByteArray(SHA1);
			}

			if (MD5 != null) {
				bw.WriteByteArray(MD5);
			}

			if (HeaderFileType != HeaderFileType.Nothing) {
				bw.Write((byte)HeaderFileType);
			}

			if (AltSize != null) {
				bw.Write((ulong)AltSize);
			}

			if (AltCRC != null) {
				bw.WriteByteArray(AltCRC);
			}

			if (AltSHA1 != null) {
				bw.WriteByteArray(AltSHA1);
			}

			if (AltMD5 != null) {
				bw.WriteByteArray(AltMD5);
			}
		...
		public void Read(BinaryReader br, List<RvDat> parentDirDats) {
			var fFlags = (FileFlags)br.ReadUInt32();
			...
			var count = (fFlags & FileFlags.HasDirDat) > 0 ? br.ReadInt32() : 0;
			_dirDats?.Clear();
			var progress = -1;
			for (var i = 0; i < count; i++) {
				var dat = new RvDat { DatIndex = i };
				dat.Read(br);
				_dirDats?.Add(dat);

				var datname = TreeFullName + @"\" + dat.GetData(RvDat.DatData.DatName);
				if (datname.Length >= 9 && datname.Substring(0, 9) == @"RomVault\") {
					datname = datname.Substring(9);
				}

				var nextProgress = (int)(br.BaseStream.Position / DB.DivideProgress);
			...
			_children?.Clear();
			for (var i = 0; i < count; i++) {
				var tChild = new RvFile((FileType)br.ReadByte()) { Parent = this };
		...
		public void DatAdd(RvFile b, bool altFile) {
			if (b.IsFile) {
				...
				if (Size == null && b.Size != null) {
					Size = b.Size;
				}

				if (CRC == null && b.CRC != null) {
					CRC = b.CRC;
				}

				if (SHA1 == null && b.SHA1 != null) {
					SHA1 = b.SHA1;
				}

				if (MD5 == null && b.MD5 != null) {
					MD5 = b.MD5;
				}

				if (AltSize == null && b.AltSize != null) {
					AltSize = b.AltSize;
				}

				if (AltCRC == null && b.AltCRC != null) {
					AltCRC = b.AltCRC;
				}

				if (AltSHA1 == null && b.AltSHA1 != null) {
					AltSHA1 = b.AltSHA1;
				}

				if (AltMD5 == null && b.AltMD5 != null) {
					AltMD5 = b.AltMD5;
				}
		...
		public EFile FileRemove() {
			if (IsFile) // File,ZippedFile or 7zippedFile
			{
				...
				if (!FileStatusIs(FileStatus.HeaderFileTypeFromDAT)) {
		...
		public void FileAdd(RvFile file, bool altFile) {
			if (file.IsFile) {
				...
				if (AltSize == null && file.AltSize != null) {
					AltSize = file.AltSize;
				}

				if (AltCRC == null && file.AltCRC != null) {
					AltCRC = file.AltCRC;
				}

				if (AltSHA1 == null && file.AltSHA1 != null) {
					AltSHA1 = file.AltSHA1;
				}

				if (AltMD5 == null && file.AltMD5 != null) {
					AltMD5 = file.AltMD5;
				}

				if (HeaderFileType == HeaderFileType.Nothing && file.HeaderFileType != HeaderFileType.Nothing) {
					HeaderFileType = file.HeaderFileType;
				}
		...
		public string SuperDatFileName() => SuperDatFileName(Dat);
		...
		public void RepStatusReset() {
			...
			if (Parent != null && (RepStatus == RepStatus.UnSet || RepStatus == RepStatus.Unknown || RepStatus == RepStatus.Ignore) &&
				...
				var datRule = ReadDat.DatReader.FindDatRule(DatTreeFullName);
				var regexList = datRule != null ? datRule.IgnoreFilesRegex : Settings.rvSettings.IgnoreFilesRegex;
				if (regexList != null) {
					foreach (var file in regexList) {
		...
		public void DirDatAdd(RvDat dat) {
			DirDatSearch(dat, out var index);
			_dirDats.Insert(index, dat);
			for (var i = 0; i < _dirDats.Count; i++) {
		...
		public RvFile Child(int index) => _children[index];

		public int ChildAdd(RvFile child) {
			ChildNameSearch(child, out var index);
		...
		public void FileStatusSet(FileStatus flag) => _fileStatus |= flag;
