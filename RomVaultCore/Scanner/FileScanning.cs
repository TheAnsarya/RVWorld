			...
			while (dbIndex < dbDir.ChildCount || fileIndex < fileDir.ChildCount) {
				...
				switch (res) {
					case 0:
						...
						var dbs = new List<RvFile>();
						var files = new List<RvFile>();
						var dbsCount = 1;
						var filesCount = 1;
						...
						for (var indexCase = (caseTest ? 0 : 1); indexCase < 2; indexCase += 1) {
							for (var indexfile = 0; indexfile < filesCount; indexfile++) {
								if (files[indexfile].SearchFound) {
									continue;
								}

								for (var indexdb = 0; indexdb < dbsCount; indexdb++) {
									if (dbs[indexdb].SearchFound) {
										continue;
									}

									var matched = Scanner.Compare.Phase1Test(dbs[indexdb], files[indexfile], EScanLevel, indexCase, out var matchedAlt);
									if (!matched) {
										continue;
									}
								...
								for (var indexdb = 0; indexdb < dbsCount; indexdb++) {
									...
									var matched = Scanner.Compare.Phase2Test(dbs[indexdb], files[indexfile], EScanLevel, indexCase, dbDir.FullNameCase, _thWrk, ref _fileErrorAbort, out var matchedAlt);
									if (!matched) {
										continue;
									}
