	...
	public static class DatUpdate {
		...
		public static void UpdateDat(ThreadWorker thWrk) {
			try {
				...
				var treeStore = new DatTreeStatusStore();
				treeStore.PreStoreTreeValue(DB.DirRoot.Child(0));

				_thWrk.Report(new bgwText("Finding Dats"));
				var datRoot = new RvFile(FileType.Dir) { Name = "RomVault", DatStatus = DatStatus.InDatCollect };
		...
		private static bool RecursiveDatTree(RvFile tDir, out int datCount) {
			datCount = 0;
			var strPath = RvFile.GetDatPhysicalPath(tDir.DatTreeFullName);
			...
			var oDir = new DirectoryInfo(strPath);

			var lFilesIn = new List<FileInfo>();

			var oFilesIn = oDir.GetFiles("*.dat");
			lFilesIn.AddRange(oFilesIn);
			oFilesIn = oDir.GetFiles("*.xml");
			lFilesIn.AddRange(oFilesIn);

			datCount += lFilesIn.Count;
			foreach (var file in lFilesIn) {
				var tDat = new RvDat();
				...
				var datRootFullName = tDat.GetData(RvDat.DatData.DatRootFullName);
				var datRule = DatReader.FindDatRule(datRootFullName);
			...
			if (tDir.DirDatCount > 1) {
				for (var i = 0; i < tDir.DirDatCount; i++) {
					tDir.DirDat(i).MultiDatsInDirectory = true;
				}
			}

			var oSubDir = oDir.GetDirectories();

			foreach (var t in oSubDir) {
				var cDir = new RvFile(FileType.Dir) { Name = t.Name, DatStatus = DatStatus.InDatCollect };
				var index = tDir.ChildAdd(cDir);

				RecursiveDatTree(cDir, out var retDatCount);
		...
		private static void RemoveOldDats(RvFile dbDir, RvFile tmpDir) {
			...
			while (dbIndex < lDir.DirDatCount || scanIndex < tmpDir.DirDatCount) {
				...
				if (dbIndex < lDir.DirDatCount && scanIndex < tmpDir.DirDatCount) {
					...
					var fileDat = tmpDir.DirDat(scanIndex);
		...
		private static bool LoadNewDat(RvDat fileDat, RvFile thisDirectory) {
			...
			var newDatFile = DatReader.ReadInDatFile(fileDat, _thWrk, out var extraDirName);
			...
			if (!string.IsNullOrWhiteSpace(extraDirName)) {
				...
				var newDirectory = new RvFile(FileType.Dir) { Dat = newDatFile.Dat };
			...
			if (MergeInDat(thisDirectory, newDatFile, out var conflictDat, true)) {
		...
		private static bool MergeInDat(RvFile dbDat, RvFile newDat, out RvDat conflict, bool checkOnly) {
			...
			while (dbIndex < dbDat.ChildCount || newIndex < newDat.ChildCount) {
				...
				if (res == 0) {
					...
					for (var indexdb = 0; indexdb < dbDatsCount; indexdb++) {
					...
					if (!checkOnly) {
						for (var indexNewDats = 0; indexNewDats < newDatsCount; indexNewDats++) {
							if (newDats[indexNewDats].SearchFound) {
								continue;
							}

							for (var indexDbDats = 0; indexDbDats < dbDatsCount; indexDbDats++) {
								if (dbDats[indexDbDats].SearchFound) {
									continue;
								}

								var matched = Compare.DatMergeCompare(dbDats[indexDbDats], newDats[indexNewDats], out var altMatch);
								if (!matched) {
									continue;
								}
		...
		private static void CheckAllDatsInternal(RvFile dbFile, string datPath) {
			...
			var dats = dbDir.DirDatCount;
			if (dats > 0) {
				for (var i = 0; i < dats; i++) {
					var testDat = dbDir.DirDat(i);
					var datFullName = testDat.GetData(RvDat.DatData.DatRootFullName);
