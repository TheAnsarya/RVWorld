...
namespace RomVaultCore.ReadDat {
	public static class ExternalDatConverter {
		...
		public static RvFile ConvertFromExternalDat(DatHeader datHeaderExternal, RvDat datFile) {
			var newDirFromExternal = new RvFile(FileType.Dir);
			var newDatFromExternal = new RvDat();
		...
		private static void CopyDir(DatDir datD, RvFile rvD, RvDat rvDat, HeaderFileType headerFileType, bool gameFile) {
			var datB = datD.ToArray();
			if (datB == null) {
				return;
			}

			foreach (var b in datB) {
				switch (b) {
					case DatDir nDir:
						var nd = new RvFile(ConvE(nDir.DatFileType)) {
						...
						if (nDir.DGame != null) {
							var dGame = nDir.DGame;
							var cGame = new RvGame();
					...
					case DatFile nFile:
						var nf = new RvFile(ConvE(nFile.DatFileType)) {
							Name = nFile.Name,
							Size = nFile.Size,
							CRC = nFile.CRC,
							SHA1 = nFile.SHA1,
		...
		private static FileType ConvE(DatFileType inft) => ConvList[(int)inft];
		...
		private static DatStatus ConvE(DatFileStatus infs) => ConvDat[(int)infs];
