...
namespace RomVaultCore.ReadDat {
	public static class DatMaker {
		...
		private static void WriteLine(string s) => _sw.WriteLine(s);
			...
			for (var i = 0; i < dir.ChildCount; i++) {
				...
				if (item.IsDir && (item.FileType == FileType.Zip || item.FileType == FileType.SevenZip || item.FileType == FileType.Dir)) {
					...
					for (var j = 0; j < item.ChildCount; j++) {
						...
						if (aDir.IsDir) {
							var dName = aDir.Name;
							for (var k = 0; k < aDir.ChildCount; k++) {
								var subFile = aDir.Child(k);
		...
		private static void ProcessDir(RvFile dir, int depth = 1) {
			var indent = new string('\t', depth); // recursive indent
			var disks = new List<string> { string.Empty };
