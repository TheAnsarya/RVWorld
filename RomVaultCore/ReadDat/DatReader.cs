...
namespace RomVaultCore.ReadDat {
	public static class DatReader {
		...
		public static DatRule FindDatRule(string datName) {
			...
			DatRule use = null;
			var longest = -1;
			var datNameLength = datName.Length;
			foreach (var s in Settings.rvSettings.DatRules) {
				if (s.DirKey.Length < 8 || s.DirKey.Substring(0, 8) != "RomVault") {
					continue;
				}

				var DirKey = "DatRoot" + s.DirKey.Substring(8) + System.IO.Path.DirectorySeparatorChar;
		...
		public static RvFile ReadInDatFile(RvDat datFile, ThreadWorker thWrk, out string extraDirName) {
			try {
				...
				var datRootFullName = datFile.GetData(RvDat.DatData.DatRootFullName);
				var fullPath = RvFile.GetDatPhysicalPath(datRootFullName);
				Debug.WriteLine("Reading Dat " + fullPath);

				DatRead.ReadDat(fullPath, ReadError, out var dh);
				if (dh == null) {
					return null;
				}

				var dirNameRule = Path.GetDirectoryName(datRootFullName) + Path.DirSeparatorChar;
				...
				var datRule = FindDatRule(dirNameRule);
		...
		private static void SetMergeType(DatRule datRule, DatHeader dh) {
			var hasRom = DatHasRomOf.HasRomOf(dh.BaseDir);
			if (hasRom) {
				var mt = datRule.Merge;
