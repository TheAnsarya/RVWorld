...
namespace RomVaultCore.FixFile {
	public static class Fix {
		public static void PerformFixes(ThreadWorker thWrk) {
			try {
				var cacheSaveTimer = new Stopwatch();
				...
				var totalFixed = 0;
				var reportedFixed = 0;
				for (var i = 0; i < DB.DirRoot.ChildCount; i++) {
				...
				for (var i = 0; i < DB.DirRoot.ChildCount; i++) {
					var tdir = DB.DirRoot.Child(i);
					var returnCode = FixDir(tdir, tdir.Tree.Checked == RvTreeRow.TreeSelect.Selected, fileProcessQueue, ref totalFixed, ref reportedFixed, cacheSaveTimer);
		...
		private static int CountFixDir(RvFile dir, bool lastSelected) {
			...
			for (var j = 0; j < dir.ChildCount; j++) {
				var child = dir.Child(j);
		...
		private static ReturnCode FixDir(RvFile dir, bool lastSelected, List<RvFile> fileProcessQueue, ref int totalFixed, ref int reportedFixed, Stopwatch cacheSaveTimer) {
			...
			var lstToProcess = new List<RvFile>();
			for (var j = 0; j < dir.ChildCount; j++) {
				lstToProcess.Add(dir.Child(j));
			}

			foreach (var child in lstToProcess) {
				var returnCode = FixBase(child, thisSelected, fileProcessQueue, ref totalFixed, ref reportedFixed, cacheSaveTimer);
		...
		private static ReturnCode FixBase(RvFile child, bool thisSelected, List<RvFile> fileProcessQueue, ref int totalFixed, ref int reportedFixed, Stopwatch cacheSaveTimer) {
			...
			var errorMessage = "";
			var returnCode = ReturnCode.LogicError;
			switch (child.FileType) {
				case FileType.Zip:
				case FileType.SevenZip:
					if (!thisSelected) {
					...
					if (!string.IsNullOrEmpty(child.FileName)) {
						var strDir = child.Parent.FullNameCase;
				...
				case FileType.Dir:
					if (thisSelected) {
						if (!string.IsNullOrEmpty(child.FileName)) {
							var strDir = child.Parent.FullName;
