	...
	public static class FixAFile {
		...
		private static void FixFileCheckName(RvFile fixFile) {
			if (!string.IsNullOrEmpty(fixFile.FileName)) {
				var sourceFullName = Path.Combine(fixFile.Parent.FullName, fixFile.FileName);
		...
		private static ReturnCode FixFileDelete(RvFile fixFile, out string errorMessage) {
			var retCode = FixFileUtils.DoubleCheckDelete(fixFile, out errorMessage);
		...
		private static ReturnCode FixFileMoveToSort(RvFile fixFile, out string errorMessage) {
			...
			returnCode = FixFileUtils.CreateToSortDirs(fixFile, out var outDir, out var toSortFileName);
			if (returnCode != ReturnCode.Good) {
				errorMessage = "";
				return returnCode;
			}

			var fixFileFullName = fixFile.FullName;
			var toSortFullName = Path.Combine(outDir.FullName, toSortFileName);

			//create new tosort record
			// FileInfo toSortFile = new FileInfo(toSortFullName);
			var toSortRom = new RvFile(FileType.File) {
				Name = toSortFileName,
				Size = fixFile.Size,
				CRC = fixFile.CRC,
			...
			returnCode = FixFileUtils.MoveFile(fixFile, toSortRom, toSortFullName, out var fileMoved, out errorMessage);
			if (returnCode != ReturnCode.Good) {
				return returnCode;
			}
			...
			if (!fileMoved) {
				...
				var fixFilePath = fixFile.FullName;
		...
		private static ReturnCode FixFileMoveToCorrupt(RvFile fixFile, out string errorMessage) {
			...
			var toSortCorruptFullName = Path.Combine(corruptDir, fixFile.Name);
			var toSortCorruptFileName = fixFile.Name;
			var fileC = 0;
			...
			var toSortCorruptRom = new RvFile(FileType.File) {
			...
			var returnCode = FixFileUtils.MoveFile(fixFile, toSortCorruptRom, toSortCorruptFullName, out var fileMoved, out errorMessage);
			if (returnCode != ReturnCode.Good) {
				return returnCode;
			}
			...
			var rvCorruptDir = new RvFile(FileType.Dir) { Name = "Corrupt", DatStatus = DatStatus.InToSort };
			var found = toSort.ChildNameSearch(rvCorruptDir, out var indexCorrupt);
			if (found != 0) {
		...
		private static ReturnCode FixFilePreCheckFixFile(RvFile fixFile, out string errorMessage) {
			...
			var testList = new List<RvFile>();

			var parent = fixFile.Parent;
			...
			if (testList.Count > 1) {
				foreach (var testChild in testList) {
					...
					switch (testFile.RepStatus) {
						case RepStatus.Delete: {
								var ret = FixFileDelete(testFile, out errorMessage);
						...
						case RepStatus.MoveToSort: {
								var ret = FixFileMoveToSort(testFile, out errorMessage);
								if (ret != ReturnCode.Good) {
									return ret;
								}
								break;
						...
						case RepStatus.MoveToCorrupt: {
								var ret = FixFileMoveToCorrupt(testFile, out errorMessage);
								if (ret != ReturnCode.Good) {
									return ret;
								}
								break;
		...
		private static ReturnCode FixFileCanBeFixed(RvFile fixFile, List<RvFile> fileProcessQueue, ref int totalFixed, out string errorMessage) {
			...
			var returnCode = FixFilePreCheckFixFile(fixFile, out errorMessage);
			if (returnCode != ReturnCode.Good) {
				return returnCode;
			}
			...
			var fixFiles = FindSourceFile.GetFixFileList(fixFile);

			if (DBHelper.IsZeroLengthFile(fixFile)) {
				var fileIn = new RvFile(FileType.File);
			...
			var fixingFile = FindSourceFile.FindSourceToUseForFix(fixFile, fixFiles);

			var fileMove = FixFileUtils.TestFileMove(fixingFile, fixFile);
			var fts = fixingFile.FullName;
