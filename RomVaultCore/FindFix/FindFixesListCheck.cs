...
namespace RomVaultCore.FindFix {
	public static class FindFixesListCheck {
		public static void GroupListCheck(FileGroup[] filegroupList) {
			foreach (var ff in filegroupList) {
		...
		private static void ListCheck(FileGroup family) {
			...
			var missingFiles = new List<RvFile>(); // files we dont have that we need

			var correctFiles = new List<RvFile>();   // files we have that are in the correct place
			var unNeededFiles = new List<RvFile>();  // files we have that are not in the correct place
			var inToSortFiles = new List<RvFile>();  // files we have that are in tosort
			var allGotFiles = new List<RvFile>();    // all files we have
			...
			foreach (var missingFile in missingFiles) {
               ...
               */
				...
				foreach (var gotFile in allGotFiles) {
			...
			foreach (var corruptFile in corruptFiles) {
			...
			foreach (var unNeededFile in unNeededFiles) {
				/*
                ...
                */
				...
				foreach (var inToSortFile in inToSortFiles) {
			...
			var canBeFixed = new List<RvFile>();
			foreach (var fLoop in files) {
				if (fLoop.RepStatus != RepStatus.CanBeFixed) {
					continue;
				}
			...
			foreach (var fOutLoop in files) {
				if (fOutLoop.RepStatus != RepStatus.NeededForFix) {
					continue;
				}

				foreach (var fInLoop in canBeFixed) {
