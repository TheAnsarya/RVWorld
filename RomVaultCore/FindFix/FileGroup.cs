	...
	public class FileGroup {
		...
		public void MergeFileIntoGroup(RvFile file) {
			...
			if (AltCRC == null && file.AltCRC != null) {
				AltCRC = file.AltCRC.Copy();
			}

			if (AltSHA1 == null && file.AltSHA1 != null) {
				AltSHA1 = file.AltSHA1.Copy();
			}

			if (AltMD5 == null && file.AltMD5 != null) {
				AltMD5 = file.AltMD5.Copy();
			}
		...
		private bool FindAltExactMatch(RvFile file) {
			...
			if (!Equal(file.Size, AltSize)) {
				return false;
			}

			if (!ArrByte.ECompare(file.CRC, AltCRC)) {
				return false;
			}

			if (!ArrByte.ECompare(file.SHA1, AltSHA1)) {
				return false;
			}

			if (!ArrByte.ECompare(file.MD5, AltMD5)) {
				return false;
			}
		...
		public static bool FindExactMatch(FileGroup fGroup, RvFile file) => fGroup.FindExactMatch(file);

		public static bool FindAltExactMatch(FileGroup fGroup, RvFile file) => fGroup.FindAltExactMatch(file);
