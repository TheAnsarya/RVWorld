...
namespace RomVaultX {
	public partial class RvTree : UserControl {
		...
		public void Setup(List<RvTreeRow> rows) {
			...
			var yPos = 0;
			var treeCount = rows.Count;
			for (var i = 0; i < treeCount; i++) {
				var pTree = new UITreeRow(rows[i]);
				_rows.Add(pTree);

				var nodeDepth = pTree.TRow.dirFullName.Count(x => x == '\\') - 1;
			...
			for (var i = treeCount - 1; i >= 0; i--) {
				var pTree = _rows[i];
				var nodeDepth = pTree.TRow.dirFullName.Count(x => x == '\\');
				if (pTree.TRow.MultiDatDir) {
					nodeDepth += 1;
				}
		...
		private void PaintTree(UITreeRow pTree, Graphics g, Rectangle t) {
			var y = pTree.RTree.Top - _vScroll;
			...
			if (recBackGround.IntersectsWith(t)) {
				var thistxt = pTree.TRow.dirName;
		...
		protected override void OnMouseDown(MouseEventArgs mevent) {
			...
			var x = mevent.X + HorizontalScroll.Value;
			var y = mevent.Y + VerticalScroll.Value;

			if (_rows != null) {
				foreach (var tDir in _rows) {
