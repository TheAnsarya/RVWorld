...
namespace TrrntZip {
	public static class TorrentZipRebuild {
		public static TrrntZipStatus ReZipFiles(List<ZippedFile> zippedFiles, ICompress originalZipFile, byte[] buffer, StatusCallback statusCallBack, LogCallback logCallback, int threadId, PauseCancel pc) {
			...
			var outputType = Program.OutZip == zipType.archive ? inputType : Program.OutZip;

			var bufferSize = buffer.Length;

			var filename = originalZipFile.ZipFilename;
			var tmpFilename = Path.Combine(Path.GetDirectoryName(filename), Path.GetFileNameWithoutExtension(filename) + ".tmp");

			var outExt = outputType == zipType.zip ? ".zip" : ".7z";
			var outfilename = Path.Combine(Path.GetDirectoryName(filename), Path.GetFileNameWithoutExtension(filename) + outExt);
			...
			var zipFileOut = outputType == zipType.zip ? new Zip() : (ICompress)new SevenZ();
			...
			try {
				...
				foreach (var t in zippedFiles) {
					...
					var zrInput = ZipReturn.ZipUntested;

					if (t.Size > 0) {
						switch (originalZipFile) {
							case Zip z:
								zrInput = z.ZipFileOpenReadStream(t.Index, false, out readStream, out streamSize, out var _);
								break;
							case SevenZ z7:
					...
					var zrOutput = zipFileOut.ZipFileOpenWriteStream(false, true, t.Name, streamSize, 8, out var writeStream);
					...
					while (sizetogo > 0) {
						...
						var sizenow = sizetogo > (ulong)bufferSize ? bufferSize : (int)sizetogo;

						fileSizeProgress += (ulong)sizenow;
						var filePercent = (int)((double)fileSizeProgress / fileSizeTotal * 20);
					...
					var crc = (uint)((CrcCalculatorStream)crcCs).Crc;
