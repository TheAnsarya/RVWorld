...
namespace TrrntZip {
	public static class TorrentZipCheck {
		public static TrrntZipStatus CheckZipFiles(ref List<ZippedFile> zippedFiles, int threadId, LogCallback statusLogCallBack) {
			...
			while (thisSortFound) {
				...
				for (var i = 0; i < zippedFiles.Count - 1; i++) {
					var c = TrrntZipStringCompare(zippedFiles[i].Name, zippedFiles[i + 1].Name);
					if (c > 0) {
						var T = zippedFiles[i];
			...
			for (var i = 0; i < zippedFiles.Count - 1; i++) {
				...
				var delete = true;
				for (var j = 0; j < zippedFiles[i].Name.Length; j++) {
		...
		public static TrrntZipStatus CheckSevenZipFiles(ref List<ZippedFile> zippedFiles, int threadId, LogCallback statusLogCallBack) {
			...
			while (thisSortFound) {
				thisSortFound = false;
				for (var i = 0; i < dirSortTest.Count - 1; i++) {
					var c = Math.Sign(string.Compare(dirSortTest[i].Name, dirSortTest[i + 1].Name, StringComparison.Ordinal));
					if (c > 0) {
						var T = dirSortTest[i];
			...
			for (var i = 0; i < dirSortTest.Count - 1; i++) {
				...
				// if they do we found an incorrect directory entry.
				var delete = true;
				for (var j = 0; j < dirSortTest[i].Name.Length; j++) {
				...
				// we found an incorrect directory so remove it.
				if (delete) {
					for (var k = 0; k < zippedFiles.Count; k++) {
			...
			while (thisSortFound) {
				thisSortFound = false;
				for (var i = 0; i < zippedFiles.Count - 1; i++) {
					var c = Trrnt7ZipStringCompare(zippedFiles[i].Name, zippedFiles[i + 1].Name);
					if (c > 0) {
						var T = zippedFiles[i];
						zippedFiles[i] = zippedFiles[i + 1];
						zippedFiles[i + 1] = T;
