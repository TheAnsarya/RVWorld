...
namespace Compress.Support.Compression.PPmd.H {
	internal class State : Pointer {
		...
		internal State Initialize(byte[] mem) => base.Initialize<State>(mem);

		internal void IncrementFreq(int dFreq) => Memory[Address + 1] = (byte)(Memory[Address + 1] + dFreq);

		internal int GetSuccessor() => Utility.readIntLittleEndian(Memory, Address + 2);

		internal void SetSuccessor(PPMContext successor) => SetSuccessor(successor.Address);

		internal void SetSuccessor(int successor) => Utility.WriteLittleEndian(Memory, Address + 2, successor);
