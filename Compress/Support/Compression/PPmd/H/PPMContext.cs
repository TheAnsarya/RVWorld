...
namespace Compress.Support.Compression.PPmd.H {
	internal class PPMContext : Pointer {
		...
		internal State getOneState() => oneState;

		internal void setOneState(StateRef oneState) => this.oneState.SetValues(oneState);
		...
		internal int getMean(int summ, int shift, int round) => (Utility.URShift((summ + (1 << (shift - round))), (shift)));

		internal void decodeBinSymbol(ModelPPM model) {
			var rs = tempState1.Initialize(model.Heap);
		...
		internal void update1(ModelPPM model, int p) {
			...
			freqData.IncrementSummFreq(4);
			var p0 = tempState3.Initialize(model.Heap);
			var p1 = tempState4.Initialize(model.Heap);
			p0.Address = p;
		...
		internal bool decodeSymbol2(ModelPPM model) {
			...
			var p = tempState1.Initialize(model.Heap);
			var temp = tempState2.Initialize(model.Heap);
		...
		internal void update2(ModelPPM model, int p) {
			var temp = tempState5.Initialize(model.Heap);
			temp.Address = p;
			model.FoundState.Address = p;
			model.FoundState.IncrementFreq(4);
			freqData.IncrementSummFreq(4);
