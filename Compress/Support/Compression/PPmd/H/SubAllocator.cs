...
namespace Compress.Support.Compression.PPmd.H {
	internal class SubAllocator {
		...
		public virtual void clean() => subAllocatorSize = 0;

		private void insertNode(int p, int indx) {
			var temp = tempRarNode;
			temp.Address = p;
			temp.SetNext(freeList[indx].GetNext());
			freeList[indx].SetNext(temp);
		}

		public virtual void incPText() => pText++;

		private int removeNode(int indx) {
			var retVal = freeList[indx].GetNext();
			var temp = tempRarNode;
			temp.Address = retVal;
		...
		private void splitBlock(int pv, int oldIndx, int newIndx) {
			...
			var p = pv + U2B(indx2Units[newIndx]);
		...
		private void glueFreeBlocks() {
			var s0 = tempRarMemBlock1;
			s0.Address = tempMemBlockPos;
			var p = tempRarMemBlock2;
			var p1 = tempRarMemBlock3;
			int i, k, sz;
			if (loUnit != hiUnit) {
				heap[loUnit] = 0;
			}
		...
		private int allocUnitsRare(int indx) {
			...
			do {
				if (++i == N_INDEXES) {
					...
					var j = FIXED_UNIT_SIZE * indx2Units[indx];
		...
		public virtual int expandUnits(int oldPtr, int OldNU) {
			var i0 = units2Indx[OldNU - 1];
			var i1 = units2Indx[OldNU - 1 + 1];
			if (i0 == i1) {
				return oldPtr;
			}
