...
namespace Compress.Support.Compression.PPmd.H {
	internal class ModelPPM {
		...
		public virtual int[] getNS2BSIndx() => NS2BSIndx;

		public virtual int[] getNS2Indx() => NS2Indx;

		private int createSuccessors(bool Skip, State p1) {
			//State upState = tempState1.Initialize(null);
			var upState = tempStateRef2;
			var tempState = tempState1.Initialize(Heap);

			// PPM_CONTEXT* pc=MinContext, * UpBranch=FoundState->Successor;
			var pc = tempPPMContext1.Initialize(Heap);
			pc.Address = minContext.Address;
			var upBranch = tempPPMContext2.Initialize(Heap);
			upBranch.Address = FoundState.GetSuccessor();

			// STATE * p, * ps[MAX_O], ** pps=ps;
			var p = tempState2.Initialize(Heap);
			var pps = 0;
		...
		private void updateModel() {
			...
			var fs = tempStateRef1;
			fs.Values = FoundState;
			var p = tempState3.Initialize(Heap);
			var tempState = tempState4.Initialize(Heap);

			var pc = tempPPMContext3.Initialize(Heap);
			var successor = tempPPMContext4.Initialize(Heap);
		...
		public int decodeChar(Decoder decoder) {
			...
			for (; ; )
			{
				...
				if (count < hiCnt) {
					byte symbol;
					var ps = tempState2.Initialize(Heap);
