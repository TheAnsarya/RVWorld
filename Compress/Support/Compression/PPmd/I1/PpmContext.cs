...
namespace Compress.Support.Compression.PPmd.I1 {
	...
	internal partial class Model {
		...
		internal struct PpmContext {
			...
			/// <returns></returns>
			public static bool operator <=(PpmContext context1, PpmContext context2) => context1.Address <= context2.Address;
		...
		private void EncodeBinarySymbol(int symbol, PpmContext context) {
			...
			var index2 = numberStatisticsToBinarySummaryIndex[context.Suffix.NumberStatistics] + previousSuccess + context.Flags + ((runLength >> 26) & 0x20);
		...
		private void EncodeSymbol2(int symbol, PpmContext context) {
			...
			uint currentSymbol;
			uint lowCount = 0;
			var index = (uint)(context.NumberStatistics - numberMasked);
...
SymbolFound:
		...
		private See2Context MakeEscapeFrequency(PpmContext context) {
			...
			if (context.NumberStatistics != 0xff) {
				...
				var index1 = probabilities[context.NumberStatistics + 2] - 3;
				var index2 = ((context.SummaryFrequency > 11 * (context.NumberStatistics + 1)) ? 1 : 0) + ((2 * context.NumberStatistics < numberStatistics + numberMasked) ? 2 : 0) + context.Flags;
		...
		private PpmContext RemoveBinaryContexts(int order, PpmContext context) {
			...
			for (var state = context.Statistics + context.NumberStatistics; state >= context.Statistics; state--) {
				if ((Pointer)state.Successor >= Allocator.BaseUnit && order < modelOrder) {
					state.Successor = RemoveBinaryContexts(order + 1, state.Successor);
				} else {
					state.Successor = PpmContext.Zero;
