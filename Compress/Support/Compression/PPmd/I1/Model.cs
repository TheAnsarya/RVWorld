...
namespace Compress.Support.Compression.PPmd.I1 {
	...
	internal partial class Model {
		...
		public Model() {
			...
			for (var index = 0; index < UpperFrequency; index++) {
			...
			emptySee2Context = new See2Context {
				Summary = (ushort)(Signature & 0x0000ffff),
				Shift = (byte)((Signature >> 16) & 0x000000ff),
				Count = (byte)(Signature >> 24)
			};
		}
...
StopDecoding:
		...
		private void StartModel(int modelOrder, ModelRestorationMethod modelRestorationMethod) {
			...
			if (modelOrder < 2) {
				...
				for (var context = maximumContext; context.Suffix != PpmContext.Zero; context = context.Suffix) {
					orderFall--;
				}
			...
			for (var index = 0; index < 256; index++) {
				var state = maximumContext.Statistics[index];
		...
		private void UpdateModel(PpmContext minimumContext) {
			...
			var currentContext = maximumContext;
			...
			var foundStateSymbol = foundState.Symbol;
			byte symbol;
			byte flag;

			var foundStateSuccessor = foundState.Successor;
...
RestartModel:
		...
		private PpmContext CreateSuccessors(bool skip, PpmState state, PpmContext context) {
			var upBranch = foundState.Successor;
			var states = new PpmState[MaximumOrder];
			uint stateIndex = 0;
