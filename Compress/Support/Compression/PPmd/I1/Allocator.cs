...
namespace Compress.Support.Compression.PPmd.I1 {
	...
	internal class Allocator {
		...
		public Allocator() => MemoryNodes = new MemoryNode[IndexCount];
		...
		public void Initialize() {
			...
			var difference = UnitSize * (AllocatorSize / 8 / UnitSize * 7);
		...
		public uint GetMemoryUsed() {
			var memoryUsed = AllocatorSize - (HighUnit - LowUnit) - (BaseUnit - Text);
		...
		public void ExpandText() {
			...
			var counts = new uint[IndexCount];
		...
		private void GlueFreeBlocks() {
			var memoryNode = new MemoryNode(LocalOffset, Memory) {
				Stamp = 0,
				Next = MemoryNode.Zero,
				UnitCount = 0
			};
			...
			while (memoryNode.Available) {
				...
				if (unitCount != 0) {
					...
					if (indexToUnits[index] != unitCount) {
						var unitCountDifference = unitCount - indexToUnits[--index];
