	...
	public partial class SevenZ {
		...
		private ZipReturn ZipFileReadHeaders() {
			try {
				...
				var mainHeader = new byte[signatureHeader.NextHeaderSize];
				...
				if (signatureHeader.NextHeaderSize != 0) {
					_zipFs.Seek(_baseOffset + (long)signatureHeader.NextHeaderOffset, SeekOrigin.Begin);
					var zr = Header.ReadHeaderOrPackedHeader(_zipFs, _baseOffset, out _header);
		...
		private void PopulateLocalFiles(out List<SevenZipLocalFile> localFiles) {
			...
			for (var i = 0; i < _header.FileInfo.Names.Length; i++) {
