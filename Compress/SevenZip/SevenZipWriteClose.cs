...
namespace Compress.SevenZip {
	public partial class SevenZ {
		private void Create7ZStructure() {
			...
			for (var i = 0; i < _packedOutStreams.Count; i++) {
			...
			for (var i = 0; i < _packedOutStreams.Count; i++) {
				...
				foreach (var v in _packedOutStreams[i].unpackedStreams) {
		...
		private void CloseWriting7Zip() {
			...
			var mainHeaderCRC = CRC.CalculateDigest(newHeaderByte, 0, (uint)newHeaderByte.Length);

			#region Header Compression
			var packedHeaderPos = _zipFs.Position;
			...
			var lzmaStreamProperties = lzs.Properties;
