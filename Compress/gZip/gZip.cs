	...
	public class gZip : ICompress {
		...
		public LocalFile GetLocalFile(int i) {
			LocalFile lf = new() {
				...
				CRC = CRC,
				IsDirectory = false,
				ModifiedTime = MTime == 0 ? null : CompressUtils.UtcTicksFromUnixDateTime((int)MTime)
			};
			return lf;
		}
		...
		private ZipReturn ZipFileReadHeaders() {
			...
			var ID1 = zipBr.ReadByte();
			var ID2 = zipBr.ReadByte();
			...
			var CM = zipBr.ReadByte();
			if (CM != 8) {
				_zipFs.Close();
				return ZipReturn.ZipUnsupportedCompression;
			}

			var FLG = zipBr.ReadByte();

			MTime = zipBr.ReadUInt32();
			var XFL = zipBr.ReadByte();
			var OS = zipBr.ReadByte();
			...
			if ((FLG & 0x8) == 0x8) {
				int XLen = zipBr.ReadInt16();
				var bytes = zipBr.ReadBytes(XLen);
			}
			...
			var gzcrc = zipBr.ReadBytes(4);
			var gzLength = zipBr.ReadUInt32();
		...
		public ZipReturn ZipFileOpenReadStream(int index, out Stream stream, out ulong streamSize) => ZipFileOpenReadStream(index, false, out stream, out streamSize);
		...
		public ZipReturn GetRawStream(out Stream st) {
			...
			var errorCode = FileStream.OpenFileRead(_zipFileInfo.FullName, out st);
			if (errorCode != 0) {
				if (errorCode == 32) {
					return ZipReturn.ZipFileLocked;
				}
