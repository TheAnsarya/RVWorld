...
namespace Compress.ZipFile {
	internal class ZipLocalFile : LocalFile {
		...
		public override ulong? LocalHead => (GeneralPurposeBitFlag & 8) == 0 ? RelativeOffsetOfLocalHeader : null;
		...
		internal ZipReturn CentralDirectoryRead(Stream zipFs, ulong offset) {
			try {
				...
				var fileCommentLength = br.ReadUInt16();
				...
				if (extraFieldLength > 0) {
					...
					var zr = ZipExtraField.ReadExtraField(extraField, bFileName, this, ref _compressedSize, ref RelativeOffsetOfLocalHeader, true);
				...
				if (fileCommentLength > 0) {
					var fileComment = br.ReadBytes(fileCommentLength);
				}
        ...
        */
		...
		internal void CentralDirectoryWrite(Stream crcStream) {
			...
			SetStatus(LocalFileStatus.Zip64,
				...
				out var headerUnCompressedSize, out var headerCompressedSize, out var headerRelativeOffsetOfLocalHeader)
				);
			_extraField = zefw.ExtraField;

			var extraFieldLength = (ushort)_extraField.Length;
			...
			var fileNameLength = (ushort)bFileName.Length;

			var versionNeededToExtract = (ushort)(GetStatus(LocalFileStatus.Zip64) ? 45 : 20);

			CompressUtils.UtcTicksToDosDateTime(HeaderLastModified, out var lastModFileDate, out var lastModFileTime);
		...
		internal ZipReturn LocalFileHeaderReadQuick(Stream zipFs) {
			try {
				...
				if (extraFieldLength > 0) {
					...
					ulong LocalHeader = 0;
					var zr = ZipExtraField.ReadExtraField(extraField, bFileName, this, ref _compressedSize, ref LocalHeader, false);
					if (zr != ZipReturn.ZipGood) {
						return zr;
					}
				}
		...
		private void LocalFileHeaderWrite(Stream zipFs) {
			...
			ZipExtraFieldWrite zefw = new();
			var zip64 = zefw.Zip64(UncompressedSize, _compressedSize, RelativeOffsetOfLocalHeader, false,
				out var headerUnCompressedSize, out var headerCompressedSize,
				out var headerRelativeOffsetOfLocalHeader);
			_extraField = zefw.ExtraField;
		...
		private void LocalFileHeaderPostCompressWrite(Stream zipFs) {
			...
			ZipExtraFieldWrite zefw = new();
			var postZip64 = zefw.Zip64(UncompressedSize, _compressedSize, RelativeOffsetOfLocalHeader, false,
				out var headerUnCompressedSize, out var headerCompressedSize, out var headerRelativeOffsetOfLocalHeader);
			var postExtraField = zefw.ExtraField;
		...
		internal ZipReturn LocalFileOpenReadStream(Stream zipFs, bool raw, out Stream readStream, out ulong streamSize, out ushort compressionMethod) {
			...
			switch (_compressionMethod) {
				...
				case 14: {
						...
						var headerSize = zipFs.ReadByte() + (zipFs.ReadByte() << 8);
		...
		private void FixFileForZip64(Stream zipFs, int oldExtraFieldLength, int newExtraFieldLength) {
			...
			var expandBy = newExtraFieldLength - oldExtraFieldLength;
		...
		private static void ExpandFile(Stream stream, long offset, long length, int extraBytes) {
			...
			var pos = length;
			while (pos > offset) {
				var toRead = pos - bufferSize >= offset ? bufferSize : (int)(pos - offset);
				pos -= toRead;
				stream.Position = pos;
