...
namespace Compress.ZipFile {
	public partial class Zip {
		...
		private ZipReturn FindEndOfCentralDirSignature() {
			...
			var buffer = new byte[buffSize + 4];
			...
			while (backPosition < maxBackSearch) {
				...
				var readSize = backPosition > buffSize + 4 ? buffSize + 4 : backPosition;
		...
		private ZipReturn EndOfCentralDirRead() {
			...
			var zipFileCommentLength = zipBr.ReadUInt16();
		...
		private ZipReturn Zip64EndOfCentralDirRead() {
			...
			var tULong = zipBr.ReadUInt64(); // Size of zip64 end of central directory record
			if (tULong != 44) {
			...
			var tUShort = zipBr.ReadUInt16(); // version needed to extract
			if (tUShort != 45) {
				return ZipReturn.Zip64EndOfCentralDirError;
			}

			var tUInt = zipBr.ReadUInt32(); // number of this disk
			if (tUInt != 0) {
				return ZipReturn.Zip64EndOfCentralDirError;
			}
		...
		private ZipReturn Zip64EndOfCentralDirectoryLocatorRead() {
			...
			var tUInt = zipBr.ReadUInt32(); // number of the disk with the start of the zip64 end of central directory
			if (tUInt != 0) {
