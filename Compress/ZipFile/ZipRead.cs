...
namespace Compress.ZipFile {
	public partial class Zip {
		...
		private ZipReturn ZipFileReadHeaders() {
			try {
				var zRet = FindEndOfCentralDirSignature();
				if (zRet != ZipReturn.ZipGood) {
					ZipFileClose();
					return zRet;
				}

				var endOfCentralDir = (ulong)_zipFs.Position;
				...
				var zip64Required = (_centralDirStart == 0xffffffff || _centralDirSize == 0xffffffff || _localFilesCount == 0xffff);
				...
				if (FileComment.Length == 22) {
					if (CompressUtils.GetString(FileComment).Substring(0, 14) == "TORRENTZIPPED-") {
						...
						var buffer = new byte[_centralDirSize];
						_zipFs.Position = (long)_centralDirStart;
						crcCs.Read(buffer, 0, (int)_centralDirSize);
						crcCs.Flush();
						crcCs.Close();

						var r = (uint)crcCs.Crc;
						crcCs.Dispose();

						var tcrc = CompressUtils.GetString(FileComment).Substring(14, 8);
						var zcrc = r.ToString("X8");
				...
				for (var i = 0; i < _localFilesCount; i++) {
					ZipLocalFile lc = new();
