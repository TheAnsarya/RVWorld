...
namespace Compress {
	public static class CompressUtils {
		public static void CreateDirForFile(string sFilename) {
			var strTemp = Path.GetDirectoryName(sFilename);
		...
		public static string GetString(byte[] byteArr) => enc.GetString(byteArr);
		...
		internal static bool IsCodePage437(string s) {
			var bOut = enc.GetBytes(s);
			var sOut = enc.GetString(bOut);

			return CompareString(s, sOut);
		}

		internal static byte[] GetBytes(string s) => enc.GetBytes(s);
		...
		public static long UtcTicksFromDosDateTime(ushort dosFileDate, ushort dosFileTime) {
			...
			var second = (dosFileTime & 0x1f) << 1;
			var minute = (dosFileTime >> 5) & 0x3f;
			var hour = (dosFileTime >> 11) & 0x1f;

			var day = dosFileDate & 0x1f;
			var month = (dosFileDate >> 5) & 0x0f;
			var year = ((dosFileDate >> 9) & 0x7f) + 1980;
		...
		public static long UtcTicksToNtfsDateTime(long ticks) => ticks - FileTimeToUtcTime;

		public static long UtcTicksFromNtfsDateTime(long ntfsTicks) => ntfsTicks + FileTimeToUtcTime;
