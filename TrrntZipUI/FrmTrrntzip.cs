...
namespace TrrntZipUI {
	public partial class FrmTrrntzip : Form {
		...
		private readonly bool UiUpdate = false;
		...
		public FrmTrrntzip() {
			...
			Text = $"TrrntzipUI.Net ({Assembly.GetExecutingAssembly().GetName().Version.ToString(3)})";
			DropBox.AllowDrop = true;
			DropBox.DragEnter += PDragEnter;
			DropBox.DragDrop += PDragDrop;

			var sval = AppSettings.ReadSetting("InZip");
			...
			if (!int.TryParse(sval, out var procc)) {
		...
		private void SetUpWorkerThreads() {
			...
			for (var i = 0; i < _threadCount; i++) {
				var threadProcess = new ThreadProcess();
				...
				var t = new Thread(cpzn.MigrateZip);
				t.Start();
		...
		private void PDragDrop(object sender, DragEventArgs e) {
			...
			var pm = new FileAdder(bccFile, file, UpdateFileCount, ProcessFileEndCallback);
			var procT = new Thread(pm.ProcFiles);
			procT.Start();
		...
		private void ProcessFileEndCallback(int processId, int fileId, TrrntZipStatus trrntZipStatus) {
			...
			if (processId == -1) {
				...
				var tGridn = new dGrid() { fileId = fileId, filename = null };
